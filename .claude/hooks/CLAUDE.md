...
### 7. Chrome DevTools MCP Suggestion
**File**: `suggest-chrome-devtools.sh`
**Triggers**: PreToolUse (Playwright MCP tools)
**Purpose**: Suggests Chrome DevTools MCP as better alternative when Playwright is used for debugging tasks

**How It Works:**
- **Advisory-only**: Uses `permissionDecision: "allow"` - never blocks, only guides
- **Smart detection**: Identifies debugging intent from tool names and parameters
- **Performance**: < 100ms execution time, minimal overhead

**Detection Patterns:**

**Strong Debugging Signals** (always suggest):
- `playwright_get_visible_html` - Getting HTML is almost always for inspection
- `playwright_console_logs` - Console logs are debugging-specific
- Chrome DevTools MCP provides better console inspection with filtering

**Conditional Signals** (parameter-based):
- `playwright_screenshot` with:
  - Empty/missing path → debugging
  - Path contains "debug", "temp", "inspect", "check" → debugging
  - Generic names: "screen.png", "page.png", "snapshot.png" → debugging
  - **Excluded**: Test artifacts ("reports/", "test-case-", "ci-cd/") → automation
- `playwright_navigate` when recent context mentions "debug", "inspect", "investigate"

**Suggested Alternatives:**
```bash
# Instead of Playwright for debugging:
playwright_get_visible_html → take_snapshot (faster, structured)
playwright_console_logs → list_console_messages (better filtering)
playwright_screenshot → take_snapshot (then screenshot if needed)
playwright_navigate → navigate_page + take_snapshot + console analysis
```

**Example Output:**
```
💡 Chrome DevTools MCP Suggestion
...
**Configuration:**
Configured in `.claude/settings.json` with matcher `mcp__playwright__*`:
```json
{
...
echo '{"tool_name": "mcp__playwright__playwright_get_visible_html", "tool_input": {}}' | \
  ./.claude/hooks/suggest-chrome-devtools.sh

# Should suggest (debug screenshot)
echo '{"tool_name": "mcp__playwright__playwright_screenshot", "tool_input": {"name": "debug.png"}}' | \
  ./.claude/hooks/suggest-chrome-devtools.sh

# Should NOT suggest (test artifact)
echo '{"tool_name": "mcp__playwright__playwright_screenshot", "tool_input": {"name": "reports/test-123.png"}}' | \
  ./.claude/hooks/suggest-chrome-devtools.sh
```

**Monitoring:**
- Hook runs before every Playwright MCP tool call
- Suggestions are advisory - Claude can choose to use Playwright anyway
- Monitor effectiveness over 2-4 weeks, adjust heuristics if needed

**Troubleshooting:**

**False Positives** (suggests when shouldn't):
- If test automation paths trigger suggestions, add to exclusion regex in hook
- Current exclusions: `report|test-case|test-report|artifact|ci-cd`
- Edit line 67 in hook to add more patterns

**False Negatives** (doesn't suggest when should):
- If debugging patterns aren't detected, add keywords to line 74
- Current patterns: `debug|temp|inspect|check`
- Be conservative - false positives are worse than false negatives

**Performance Issues:**
- Hook should execute in < 100ms
- If slower, check `jq` is installed and working
- Graceful degradation: exits silently if `jq` missing
